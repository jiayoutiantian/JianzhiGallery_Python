...
#    self.initializer = tf.random_uniform_initializer(-1,1)
    ...
    # num_images is the number of images in one outfit, default is 6.
    self.images = None
    self.TRAIN_LOG_DIR = os.path.join('Log/train/', time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
    ...
    self.loss_mask = None
    self.loss_mask_weights = None
    self.weights_ave = None
    self.votes = None
...
#        votes = np.ceil(votes/100)+1
        ...
        images_and_captions.append([set_id, images, image_ids, captions, weights, votes])
      ...
      (set_ids, image_seqs, image_ids, input_mask,
       loss_mask, cap_seqs, cap_mask, weights, votes) = (
...
#      pdb.set_trace()
  ...
  def build_model(self):
    ...
    norm_image_embeddings = norm_image_embeddings[:,0:6,:]
#    self.image_embeddings = tf.Print(self.image_embeddings,[self.image_embeddings],message="This is image_embeddings: ")
#    norm_image_embeddings = tf.Print(norm_image_embeddings,[norm_image_embeddings],message="This is norm_image_embeddings: ")

    norm_seq_embeddings = tf.nn.l2_normalize(self.seq_embeddings, 2)
    norm_seq_embeddings = norm_seq_embeddings[:,0:6,:]
    norm_seq_embeddings = (
        tf.pad(norm_seq_embeddings, [[0, 0],
               [0, self.config.number_set_images - tf.shape(norm_seq_embeddings)[1]],
               [0, 0]], name="norm_seq_embeddings"))
#    self.seq_embeddings = tf.Print(self.seq_embeddings,[self.seq_embeddings],message="This is seq_embeddings: ")
#    norm_seq_embeddings = tf.Print(norm_seq_embeddings,[norm_seq_embeddings],message="This is norm_seq_embeddings: ")
    if self.mode == "inference":
      pass
    else:
...
#      pdb.set_trace()
      emb_loss_mask = tf.greater(tf.reduce_sum(self.cap_mask, 1), 1)
      # Image mask is padded it to max length.
      emb_loss_mask = emb_loss_mask[:,0:self.config.number_set_images]
...
#      norm_image_embeddings = tf.Print(norm_image_embeddings,[norm_image_embeddings],message="This is norm_image_embeddings 1: ")
#      emb_loss_mask = tf.Print(emb_loss_mask,[emb_loss_mask],message="this is emb_loss_mask:")
      norm_image_embeddings = tf.boolean_mask(norm_image_embeddings,
                                                emb_loss_mask)
#      norm_image_embeddings = tf.Print(norm_image_embeddings,[norm_image_embeddings],message="This is norm_image_embeddings 2: ")
...
#      cost_im = tf.Print(cost_im, [cost_im], message="This is cost_im: ")
...
#      emb_batch_loss = tf.Print(emb_batch_loss, [emb_batch_loss], message="This is emb_batch_loss: ")
    ...
    if self.mode == "inference":
      ...
      self.input_mask = self.input_mask[:,0:self.config.number_set_images + 1]
      ...
      temp_shape = tf.ones([1, tf.shape(self.loss_mask)[1]])
#      sess = tf.Session()
#      print(sess.run(self.loss_mask))
#      self.loss_mask= tf.Print(self.loss_mask,[self.loss_mask],message='this is the new tensor:')
      self.loss_mask = self.loss_mask[:,0:6]
      ...
      self.weights = self.weights * self.votes
      self.weights = tf.reshape(self.weights,[self.config.batch_size, 1])
      self.weights = tf.tile(self.weights,[1,tf.shape(loss_mask)[1]])

      self.loss_mask_weights =self.weights*loss_mask
      loss_mask_weights = self.loss_mask_weights
      loss_mask_weights=tf.Print(loss_mask_weights,[loss_mask_weights],message='the loss_mask_weights element is:')
      loss_mask = tf.reshape(tf.to_float(loss_mask),
                    [self.config.number_set_images * self.config.batch_size,1])
      loss_mask_weights = tf.reshape(tf.to_float(loss_mask_weights),
                    [self.config.number_set_images * self.config.batch_size,1])      
      # Forward rnn.
...
#      loss_mask =loss_mask* self.weights
#      loss_mask = tf.Print(loss_mask, [loss_mask], message="This is loss loss_mask: ")
      loss_mask = tf.squeeze(loss_mask)
      loss_mask_weights = tf.squeeze(loss_mask_weights)
...
#                           tf.reduce_sum(loss_mask)+1e-7,
#                           name="f_lstm_loss")
      

      f_lstm_loss = tf.Print(f_lstm_loss, [f_lstm_loss], message="This is before loss f_lstm_loss: ")
      gg=tf.ones([tf.shape(f_lstm_loss)[0]-4])
      tt= tf.shape(f_lstm_loss)[0]-4
#      tt = tf.Print(tt, [tt], message="This is loss tt: ")
#      self.weights = tf.concat([self.weights,self.votes],0)
#      loss_mask = loss_mask * self.weights
#      loss_mask = tf.Print(loss_mask, [loss_mask], message="This is loss loss_mask: ")
#      loss_mask_weights = tf.Print(loss_mask_weights, [loss_mask_weights], message="This is loss loss_mask_weights: ")
      loss_mask_weights = tf.boolean_mask(loss_mask_weights,
                                           tf.cast(loss_mask, tf.bool))
#      sess = tf.InteractiveSession()
#      print(sess.run(loss_mask_weights))
#      f_lstm_loss = tf.div(tf.reduce_sum(f_lstm_loss*loss_mask_weights),
#                           tf.reduce_sum(loss_mask),
#                           name="f_lstm_loss")
      f_lstm_loss = tf.div(tf.reduce_sum(f_lstm_loss),
                           tf.reduce_sum(loss_mask),
                           name="f_lstm_loss")
#      f_lstm_loss = tf.Print(f_lstm_loss, [f_lstm_loss], message="This is after loss f_lstm_loss: ")

#      gg = self.weights * self.votes
#      tt = gg[0]
#      tt = tf.Print(tt, [tt], message="This is loss tt: ")
#      self.weights_ave = tf.reduce_sum(tf.cast(self.weights,tf.float32))/self.config.batch_size
#      f_lstm_loss = f_lstm_loss * self.weights_ave * self.votes
#      f_lstm_loss = tf.Print(f_lstm_loss, [f_lstm_loss], message="This is before loss f_lstm_loss1: ")
#      f_lstm_loss = f_lstm_loss * self.weights_ave
#      f_lstm_loss = tf.Print(f_lstm_loss, [f_lstm_loss], message="This is loss f_lstm_loss1: ")
      # Backward rnn.
...
#                           tf.reduce_sum(loss_mask)+1e-7,
#                           name="b_lstm_loss")
#      b_lstm_loss = tf.div(tf.reduce_sum(b_lstm_loss*loss_mask_weights),
#                           tf.reduce_sum(loss_mask),
#                           name="b_lstm_loss")
#      sess = tf.Session()
#      print(sess.run(loss_mask))
#      print(sess.run(b_lstm_loss))
      b_lstm_loss = tf.div(tf.reduce_sum(b_lstm_loss),
                           tf.reduce_sum(loss_mask),
                           name="b_lstm_loss") 
      b_lstm_loss = tf.Print(b_lstm_loss, [b_lstm_loss], message="This is before loss b_lstm_loss: ")
      emb_batch_loss = tf.Print(emb_batch_loss, [emb_batch_loss], message="This is before loss emb_batch_loss: ")

#      b_lstm_loss = b_lstm_loss * self.weights_ave * self.votes
#      b_lstm_loss = b_lstm_loss * self.weights_ave 
      
#      if np.isnan(b_lstm_loss[0]):
#         set_id = tf.Print(set_id, [set_id], message="This is set_id to nan: ")
...
#        print(f_lstm_loss)
#        cur = f_lstm_loss[0]
#        f_lstm_loss = tf.Print(f_lstm_loss, [f_lstm_loss], message="This is f_lstm_loss: ")
#        self.config.f_rnn_loss_factor=tf.Print(self.config.f_rnn_loss_factor,[self.config.f_rnn_loss_factor])
        tf.losses.add_loss(f_lstm_loss * self.config.f_rnn_loss_factor)
      if self.config.b_rnn_loss_factor > 0:
 #       pdb.set_trace()
     #   print(b_lstm_loss)
#        b_lstm_loss = tf.Print(b_lstm_loss, [b_lstm_loss],message="This is b_lstm_loss: ")
#        self.config.b_rnn_loss_factor = tf.Print(self.config.b_rnn_loss_factor,[self.config.b_rnn_loss_factor])
        tf.losses.add_loss(b_lstm_loss * self.config.b_rnn_loss_factor)
     
 ## self compute losses 
#      aa= f_lstm_loss * self.config.f_rnn_loss_factor + b_lstm_loss * self.config.b_rnn_loss_factor
#      aa=tf.Print(aa,[aa])
...
#              set_id = tf.Print(set_id, [set_id], message="This is set_id to nan: ")
      ...
      tf.summary.scalar("lstm_loss",(f_lstm_loss * self.config.f_rnn_loss_factor + b_lstm_loss * self.config.b_rnn_loss_factor))
      tf.summary.scalar("total_loss", total_loss)
      write_op = tf.summary.merge_all()
      '''
      op = tf.summary.scalar("total_loss", total_loss)
      op = tf.Print(op,[total_loss], "total_loss")
      self.summary_ops.append(op)
      op = tf.summary.scalar("f_lstm_loss", f_lstm_loss)
      op = tf.Print(op,[f_lstm_loss], "f_lstm_loss")
      self.summary_ops.append(op)
      op = tf.summary.scalar("b_lstm_loss", b_lstm_loss)
      op = tf.Print(op,[b_lstm_loss], "b_lstm_loss")
      '''


      self.summary_ops.append(write_op)
      for var in tf.trainable_variables():
        tf.summary.histogram(var.op.name, var)
#      self.train_writer.add_summary(merged)
 ...
 #     saver = tf.train.import_meta_graph('model/Classifier/inceptionV3_tf.ckpt.meta')
